...
type WithdrawChannel struct {
	...
	CurrencyCode          string `gorm:"column:currency_code;type:varchar(500);not null;comment:'币种代码，如\"USDT\"、\"CNY\"等';index:idx_withdraw_channels_currency" json:"currency_code"`              // 币种代码
	WithdrawChannelName   string `gorm:"column:withdraw_channel_name;type:varchar(500);not null;comment:'提现通道名称，用于前端展示'" json:"withdraw_channel_name"`                                             // 提现通道名称
	WithdrawChannelCode   string `gorm:"column:withdraw_channel_code;type:varchar(500);not null;comment:'提现通道编码，系统内部唯一标识';index:idx_withdraw_channels_code_currency" json:"withdraw_channel_code"` // 提现通道编码
