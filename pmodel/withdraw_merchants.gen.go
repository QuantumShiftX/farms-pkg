...
type WithdrawMerchant struct {
	...
	Name                  string `gorm:"column:name;type:varchar(500);not null;comment:三方代付平台名称" json:"name"`                                         // 三方代付平台名称
	CurrencyCode          string `gorm:"column:currency_code;type:varchar(500);not null;comment:币种编码" json:"currency_code"`                           // 币种编码
	...
	MerchantNo            string `gorm:"column:merchant_no;type:varchar(500);not null;comment:商户号" json:"merchant_no"`                                // 商户号
	Key                   string `gorm:"column:key;type:varchar(500);not null;comment:密钥" json:"key"`                                                 // 密钥
	ExternalInfo          string `gorm:"column:external_info;type:text;not null;comment:扩展信息" json:"external_info"`                                   // 扩展信息
	WithdrawOrderPayURL   string `gorm:"column:withdraw_order_pay_url;type:varchar(500);not null;comment:下单地址(提现下单地址)" json:"withdraw_order_pay_url"` // 下单地址(提现下单地址)
	WithdrawOrderQueryURL string `gorm:"column:withdraw_order_query_url;type:varchar(500);not null;comment:订单查询地址" json:"withdraw_order_query_url"`   // 订单查询地址
	BalanceQueryURL       string `gorm:"column:balance_query_url;type:varchar(500);not null;comment:余额查询地址" json:"balance_query_url"`                 // 余额查询地址
	SuccessTag            string `gorm:"column:success_tag;type:varchar(500);default:success;comment:提现成功标识(回调成功标识)" json:"success_tag"`              // 提现成功标识(回调成功标识)
	CallbackAddress       string `gorm:"column:callback_address;type:varchar(500);comment:回调IP地址(多个用逗号分隔)" json:"callback_address"`                   // 回调IP地址(多个用逗号分隔)
