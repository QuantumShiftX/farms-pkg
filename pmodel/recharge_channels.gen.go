...
type RechargeChannel struct {
	...
	CurrencyCode          string `gorm:"column:currency_code;type:varchar(255);not null;comment:'币种代码，如\"USDT\"、\"CNY\"等';index:idx_recharge_channels_currency" json:"currency_code"`              // 币种代码
	RechargeChannelName   string `gorm:"column:recharge_channel_name;type:varchar(255);not null;comment:'充值通道名称，用于前端展示'" json:"recharge_channel_name"`                                             // 充值通道名称
	RechargeChannelCode   string `gorm:"column:recharge_channel_code;type:varchar(255);not null;comment:'充值通道编码，系统内部唯一标识';index:idx_recharge_channels_code_currency" json:"recharge_channel_code"` // 充值通道编码
	...
	IFSC                  string `gorm:"column:ifsc;type:varchar(255);comment:'银行IFSC代码，印度银行系统使用的分行识别码'" json:"ifsc"`                                                                              // 银行IFSC代码
